class DropBoxController{
    ...
    constructor(){
            ...
            // pasta inicial
        this.currentFolder = ['pasta'];

        // cria um novo evento mudando sua seleção
        this.onselectionchange = new Event('selectionchange');

       this.navEl = document.querySelector('#browse-location');

       // coleta o id dentro do html do botão de enviar arquivos
        this.btnSendFile =document.querySelector("#btn-send-file");
        
        // coleta o id dentro do html do enviar(input) de arquivos
        this.inputFiles = document.querySelector("#files");
       
        // coleta o id dentro do html da barra de carregar
        this.snackModal = document.querySelector("#react-snackbar-root");
        
        // pega o progresso da barra direto do css da classe do id snackmodal
        this.progressBar = this.snackModal.querySelector(".mc-progress-bar-fg");
        
        //pegando o class do nome dos arquivos sendo enviados na barra
        this.nameFile = this.snackModal.querySelector(".filename");
        
        //pegando o class do tempo dos arquivos sendo enviados na barra
        this.timeFile = this.snackModal.querySelector(".timeleft");

        // pega o id do lista e dos diretórios 
  ...
  var firebaseConfig = {
    apiKey: "AIzaSyBLzHmOj-x52nTqtVbVFgSNxd6sI1keI48",
    authDomain: "dropbox-clone-6d3f4.firebaseapp.com",
    databaseURL: "https://dropbox-clone-6d3f4-default-rtdb.firebaseio.com",
    projectId: "dropbox-clone-6d3f4",
    storageBucket: "dropbox-clone-6d3f4.appspot.com",
    messagingSenderId: "37199457013",
    appId: "1:37199457013:web:af2fb3d077a98abe3d33bc",
    measurementId: "G-9V0X23PVRH"
  }
  // Initialize Firebase
  ...
  firebase.storage();
   ...
   getSelection(){
    //retorna todos que estão selecionados
   ...
   removeTask(){
    ...
    this.getSelection().forEach(li=>{
        ...
        promises.push(new Promise ((resolve, reject)=>{
            ...
            if(file.type === 'folder'){
                ...
                this.removeFolderTask(this.currentFolder.join('/'), file.name).then(()=>{
                     ...
                     //resolve com fields da key:key nos botões direto no firebase
                    resolve({
                         fields:{
                            key
                                 }
     ...
     //retorna todos os arquivos com as promessas
    ...
    removeFile(ref, name){
        ...
        //delete() metodo do storage firebase
         return fileRef.delete()

    };
   
    // metodo que inicia os botões
    initEvents(){

            //evento para criar pasta
            this.btnNewFolder.addEventListener('click', e=>{

               let nameFolder = prompt('nome da pasta:');
               if(nameFolder){
        ...
        this.btnRename.addEventListener('click', e =>{
            ...
            //pega o arquivo com as informações, transformando em objeto 
            let file = JSON.parse(li.dataset.file);

            // aparece um prompt para renomear o arquivo usando o file.name
            let name = prompt('renomear o arquivo:', file.name);

           //se clicou no renomear vai adicionar no ${file.name} o name novo
            if(name){
            file.name = name
            
            // pega no firebase  pega a pasta filho usando a key e coloca no file
        ...
        //cria o evento click em cima do botão e envia com o input
       this.btnSendFile.addEventListener('click', event =>{
                this.inputFiles.click()       
        });
        // pega o input e altera o arquivo
        this.inputFiles.addEventListener("change", event =>{

             //desativar o botão enquanto é feito o upload
            ...
            this.uploadTask(event.target.files).then( responses =>{
               ...
               this.toggleDisplay(this.snackModal);
            })
    ...
    updateComplete(){
         ...
         //pega a barra de load e mostra ela.
         this.toggleDisplay(this.snackModal);
         //zera o campo
         this.inputFiles.value = "";
         //ativando o botão novamente após o upload
         this.btnSendFile.disabled = false;
      
    }
    //metodo que localiza a barra e mostra ou esconde
    toggleDisplay(element) {
        var newStyle = (element.style.display == 'block') ? 'none' : 'block'
        element.style.display = newStyle
      } 
   
      //metodo que pegará os files e irá fazer um upload com uma ou várias promessas
   ajax(method = 'GET', url, formData = new FormData(), onprogress = function(){}, onloadstart = function(){}){
    return new Promise((resolve, reject) =>{

        let ajax = new XMLHttpRequest();
               // o rota que esse ajax ira funcionar primeiro o metodo depois a url
                ajax.open(method, url);
              //fazendo um onload do ajax dizendo o que ele irá realizar
                ajax.onload = event =>{
                    try{
    ...
    uploadTask(files){         
       ...
       //[...params] cria um array com varios objetos e dai fazendo um forEach
        [...files].forEach(file =>{
    ...
    // retorna o arquivos de todas as promessas
      return Promise.all(promises);
    }
   
   //metodo que pega tempo do load dos arquivos 
    uploadProgress(event, file){
        //os let events 
        // tempo que falta menos tempo inicial
   ...
   let loaded = event.loaded;
    ...
    // esse let pega o load divide pelo total vezes 100, isso é regra de tres 
      let porcent = parseInt((loaded / total) * 100);
    
    //   tempo que falta
    let timeleft = ((100 - porcent) * timespent) / porcent ;

    // pega o progresso da barra e altera com porcentagem certa do load ocorrendo em tempo real
    this.progressBar.style.width = `${porcent}%`;

    this.nameFile.innerHTML = file.name ;
    this.timeFile.innerHTML =  this.formatTime(timeleft);
    ...
    getFirebaseRef(path){
            //join faz com que se ponha barra entre os files no array
    ...
    `;
     ...
     getFileView(file, key){
         ...
         li.dataset.key = key;
         // fazer um json pois o dataset só extrai string e o file é objeto
    ...
    readFiles(){
        ...
        this.getFirebaseRef().on('value', snapshot =>{
            ...
            snapshot.forEach(snapshotItem =>{
                ...
                if(data.type){
                // pegando o listFiles que é o id e adicionando com appendDhild
                // pego o metodo getFIleView para o seu nome e o seu tipo
    ...
    initEventsLi(li){
        ...
        //evento de duplo click em cima do li = arquivos na nuvem
       li.addEventListener('dblclick', e=>{
        ...
        //adiciona o evento click e faz o classList.toggle() para clicar no seu espaço 
        li.addEventListener('click', e=>{


            // detecta se for pressionado shift
            if(e.shiftKey){
                // pega o primeiro selecionado
                let firstLi = this.listFiles.querySelector('.selected');
                // if para pegar o primeiro selecionado 
                if(firstLi){

                    let indexStart;
                    let indexEnd;
                    li.parentElement.childNodes.forEach((el, index) =>{
                        // forEach pega o primeiro e o ultimo
                        if(firstLi === el) indexStart =index;
                        if(li === el) indexEnd =index;

                    });
                    //pega o primeiro e o ultimo e os ordena .sort()
                    let index = [indexStart, indexEnd].sort();
                    //forEach para detectar os arquivos selecionados e adiciona-los com shift
                    li.parentElement.childNodes.forEach((el,i) =>{
                       // adiciona as pastas acima ou abaixo dos index adicionados
            ...
            // se não estiver pressionado o CTRL ele pega selecionado e remove o anterior
            if(!e.ctrlKey){
            ...
            //executa o selecet nos arquivos 
